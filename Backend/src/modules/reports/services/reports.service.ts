...
private async validatePayload(dto: CreateReportDto) {
 ...
 async findAll(query: ReportQueryDto) {
    let {  category,  status,  sort,  sortOrder,  page = 1,  limit = 10,  minConfidence,} = query as any;
    ...
    if (minConfidence) {
      queryBuilder.andWhere(
        'report.confidenceScore >= :min',
        { min: minConfidence },
      );
    }

    const allowedSortFields = [ 'createdAt', 'status', 'category', 'confidenceScore', ];
